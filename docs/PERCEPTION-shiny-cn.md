...
- **颜色**：蓝-白-红发散——蓝 = 预测耐药（低杀伤），红 = 预测敏感（高杀伤）
- **点大小**：克隆占比（占比大的克隆点大）
...
>
> 1. 耐药标志物线索：最右下角的克隆中，SLC2A1 的高表达区与厄洛替尼预测低杀伤力区在空间上存在重叠，提示 SLC2A1 高表达可能是该耐药克隆的标志物。
> 2. 敏感主群体：位于正上方的几个主要细胞群，SLC2A1 表达接近于零，却恰好是预测药物杀伤率最高的区域。
>
...
**Q4：为什么杀不死？预测分数和我想的不一样？**
预测分数是模型基于 DepMap 训练得到的相对排名（已归一化到 0–1），"1" 表示在该批次克隆中最敏感，不代表绝对杀伤比例。多克隆异质性、耐药通路激活都会拉低分数。
