...
- Initial release of the R implementation of the PERCEPTION approach
  (predicting personalized drug response from single-cell
  transcriptomics).
- Train drug-response models on large-scale cell-line screens
  (`train_models`) or load pre-trained models (`load_model`).
...
- Shiny web application (`run_perception_app`) covering the whole
  pipeline.
