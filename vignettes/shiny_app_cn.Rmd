...
![模型 Validation ROC 曲线（Train 页）](../man/figures/shiny-validation-roc.png)

**Validation ROC** 展示训练好的模型在三个验证数据集（Bulk / Pseudo-bulk / Single-cell）上区分 Responder 与非 Responder 的能力，含各数据集 AUC——AUC 越高说明临床分层能力越强。与 Performance Plot 一样，仅对 Train 页训练 / `train_models()` 产出的模型可用。
...
![UMAP 基因表达](../man/figures/shiny-gene-expression.png)

基因表达视图按每个细胞的单基因表达量着色。数值经 5th–95th 百分位压缩到 [0, 1]，再以灰→红渐变显示：灰色表示无或低表达，红色表示高表达。用来查看基因在哪些细胞群中高表达。

![UMAP 药物活力](../man/figures/shiny-drug-viability.png)

药物活力视图按每个细胞所属克隆的预测活力分数着色，形成拼接色块：红色块为预测耐药（高活力），蓝色块为预测敏感（低活力）。它显示耐药与敏感亚克隆在嵌入空间中的位置。

![UMAP 克隆身份（全局聚类）](../man/figures/shiny-clone-identity.png)
...
两者不同。应用侧上限是 1 GB；而 **413 是应用前面的反向代理**在请求到达 R 之前返回的，通常是 nginx 的 `client_max_body_size` 默认值 1 MB —— 所以表现为**表达矩阵传不上去（几十到几百 MB），而 mapping / response 两张小表正常**。若服务器由你管理，在代理配置里放开：
...
另外，减少需要传输的字节数同样有效：把矩阵存成 `.csv.gz` / `.tsv.gz` / `.txt.gz` 上传（gzip 通常能压到 1/3 ~ 1/5），或用本身就是压缩格式的 `.rds`。DepMap 优先用 "Download & Load" 按钮（服务器直下，不经浏览器）。
...
**Q5：关闭应用后数据会留下吗？**
会保留。DepMap 数据缓存在持久化目录（Windows：用户数据目录，Linux 可用环境变量 `PERCEPTIONX_DEPMAP_CACHE_DIR` 指定）。应用关闭后文件仍留在磁盘，且**永远不会被自动删除**——服务器管理员预先下载好的缓存会被所有用户直接复用，约 567 MB 的下载只会发生一次。
