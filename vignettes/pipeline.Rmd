...
PERCEPTIONx implements the PERCEPTION approach (PERsonalized single-Cell Expression-based Planning for Treatments In ONcology), which predicts how individual patients respond to drug treatments by leveraging both bulk and single-cell RNA sequencing data. This vignette walks through the complete pipeline:
...
PERCEPTIONx relies on DepMap bulk expression, single-cell expression, and drug response (AUC) data. The `load_depmap()` function downloads and caches these datasets automatically.
...
>
> PERCEPTIONx models are trained on **rank-normalized** expression data. If you
...
PERCEPTION-shiny is the interactive web dashboard (built with Shiny) that
...
A ready-to-run end-to-end pipeline script lives in the source repository under
`tools/` (it is not shipped with the installed package). After cloning the repo, run:
