...
The result is a matrix with cells as rows and drugs as columns, where lower values (lower viability) indicate higher drug sensitivity.
...
a rich tooltip (clone id, viability score, proportion, FPR/TPR, ...):
...
background worker processes, so the UI never blocks other users; the main
process keeps only lightweight DepMap metadata. See `docs/PERCEPTION-shiny.md`
for the async architecture and the `PERCEPTION_WORKERS` /
