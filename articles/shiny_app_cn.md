...
**Q1：上传提示 “Maximum upload size exceeded”，或返回 HTTP 413？**
两者不同。应用侧上限是 1 GB；而 **413 是应用前面的反向代理**在请求到达 R
之前返回的，通常是 nginx 的 `client_max_body_size` 默认值 1 MB ——
所以表现为**表达矩阵传不上去（几十到几百 MB），而 mapping / response
两张小表正常**。若服务器由你管理，在代理配置里放开：

``` nginx
server {
    client_max_body_size 1024m;   # 默认是 1m
    client_body_timeout  600s;
    location /shinyapps/ {
        proxy_pass http://127.0.0.1:3838/;
        proxy_request_buffering off;   # 大文件直接透传，不落盘缓冲
        proxy_read_timeout 3600s;
    }
}
```

另外，减少需要传输的字节数同样有效：把矩阵存成 `.csv.gz` / `.tsv.gz` /
`.txt.gz` 上传（gzip 通常能压到 1/3 ~ 1/5），或用本身就是压缩格式的
`.rds`。DepMap 优先用 “Download & Load” 按钮（服务器直下，不经浏览器）。
...
`PERCEPTIONX_DEPMAP_CACHE_DIR`
指定）。应用关闭后文件仍留在磁盘，且**永远不会被自动删除**——服务器管理员预先下载好的缓存会被所有用户直接复用，约
567 MB 的下载只会发生一次。
