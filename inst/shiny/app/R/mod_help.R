    ...
    adv_pages <- list(
      ...
      tw_slide("figures/shiny-predict-heatmap.png",
        ...
        list(p("On the Predict tab click ", tw_b("Run Prediction"), ". The right panel is the deliverable: a clone-level table and heatmap plus the patient-level predictions."),
             p("The defaults are right for most users: ", tw_b("Model Source"), " = From Training/Demo and ", tw_b("Expression Data"), " = From Data Tab. Change them only to use an uploaded .RDS model or a different expression file."),
             p("Then choose the ", tw_b("Aggregation Mode"), " - how clone scores collapse into one patient score:")),
