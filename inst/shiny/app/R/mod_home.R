...
mod_home_server <- function(id, shared, main_session) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$go_demo, {
      ...
      # 1. Raw expression matrix (NOT rank-normalized — prepare_data will handle)
      # Use numeric (not integer) to avoid Seurat LogMap issues
      expr_matrix <- matrix(as.numeric(runif(length(gene_names) * n_cells, 0.1, 50)),
                            nrow = length(gene_names), ncol = n_cells)
      rownames(expr_matrix) <- gene_names
      colnames(expr_matrix) <- cell_names

      # 2. Patient-Cell mapping
      ...
      # 3. Clinical response — 10 Responders + 10 Non-responders
      ...
      # 4. Run prepare_data (Seurat clustering + rank normalization)
      w <- Waiter$new(
        html = tagList(
      ...
      tryCatch({
        ...
        shared$user_expr <- expr_matrix  # Keep cell-level expression for UMAP gene plots
        shared$user_clones <- prepared$cell_clone_map
        w$hide()

        # 5. Distinct models for two drugs (different seeds → different predictions)
        if (requireNamespace("caret", quietly = TRUE) && requireNamespace("glmnet", quietly = TRUE)) {
          make_drug_model <- function(drug_name, seed) {
            set.seed(seed)
            n_train <- 80
            x_train <- matrix(rnorm(n_train * length(gene_names)), n_train, length(gene_names))
            colnames(x_train) <- gene_names
            y_train <- rnorm(n_train, mean = sample(c(-0.3, 0.2), 1), sd = sample(c(0.5, 0.8, 1.0), 1))
            ...
            obj <- list(
              ...
              single_best = sample(gene_names, 1)
            )
          ...
          shared$models <- list(
            abemaciclib = make_drug_model("abemaciclib", 101),
            erlotinib = make_drug_model("erlotinib", 202)
          )
